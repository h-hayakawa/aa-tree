...
static
aatree_node *__aa_tree_delete__(struct __del_info__ *info, aatree_node *tree, int32_t *flag)
{
  int32_t cmp;
  if (tree == &nil){
    return tree;
...
void aa_tree_print(FILE *fp, aatree_node *tree){
  if (tree == NULL || tree == &nil){
    return;
  }
  aa_tree_print(fp, tree->left);
  fprintf(fp, "%d\n", tree->val);
  aa_tree_print(fp, tree->right);
}
static
